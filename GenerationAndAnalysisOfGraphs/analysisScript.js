const jsonDataUrl = './directed/graph5.json';
    ...
    fetch(jsonDataUrl)
      ...
      .then(jsonData => {
        ...
        text.innerHTML += "<br>Shortest path from node 0 to node 4 is: " + path.get(0).get(4).join(" -> ");
        text.innerHTML += "<br>Number of cliques: " + res3;
        text.innerHTML += "<br>Size of maximal clique: " + res2;



        const links = jsonData.map(({ u, v, w }) => ({ source: u, target: v, weight: w }));
        const nodes = Array.from(new Set(jsonData.flatMap(({ u, v }) => [u, v])), id => ({ id, color: getRandomColor() }));
        ...
        function highlightMaxClique() {
          const maxClique = findMaxClique();
          if (maxClique) {
            node.filter(d => maxClique.includes(d.id))
              .classed('highlight', true);
          }
        }

        function highlightCompleteSubgraph() {
          const completeSubgraph = findCompleteSubgraph();
          if (completeSubgraph) {
            node.filter(d => completeSubgraph.includes(d.id))
              .classed('highlight', true);
          }
        }
        ...
        function findMaxClique() {
          ...
          for (const [nodeId, neighbors] of adjacencyList.entries()) {
            ...
            for (const neighbor of neighbors) {
              if (clique.every(node => adjacencyList.get(node).includes(neighbor))) {
                clique.push(neighbor);
              }
            }
            if (clique.length > maxCliqueSize) {
              maxClique = clique;
              maxCliqueSize = clique.length;
            }
          }
          return maxClique;
        }
        ...
        function findCompleteSubgraph() {
          for (const [nodeId, neighbors] of adjacencyList.entries()) {
            const completeSubgraph = [nodeId];
            if (neighbors.every(neighbor => adjacencyList.get(neighbor).includes(nodeId))) {
              neighbors.forEach(neighbor => completeSubgraph.push(neighbor));
